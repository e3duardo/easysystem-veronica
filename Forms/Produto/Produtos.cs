    ...
    public partial class Produtos : UserControl
    {
        ...
        public List<Library.Classes.QItemProduto> produtos
        {
            get
            {
                ...
                foreach (DataGridViewRow row in this.produtoDGV.Rows)
                {
                    Library.Produto produto = Library.ProdutoBD.FindById((long)row.Cells[0].Value);
        ...
        private void adicionarProdutoButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    produto = Library.ProdutoBD.FindById(produtoId);
                }
                ...
                if (quantidade != 0 & produto != null)
                {
                    ...
                    if (this.CheckProdutoAdicionado(produto.Id, out gridRowIndex))
                    {
                        ...
                        decimal precoAvista = produto.PrecoVendaAVista;

                        if (moneyTextBox1.Value != 0)
                        {
                            preco = moneyTextBox1.Value.GetValueOrDefault(0);
                            precoAvista = moneyTextBox1.Value.GetValueOrDefault(0);
                        }
                        if (percentTextBox1.Value != 0)
                        {
                            decimal desconto = this.percentTextBox1.Value;

                            preco = preco - (preco * (desconto / 100));
                            precoAvista = precoAvista - (precoAvista * (desconto / 100));
                        }

                        this.produtoDGV.Rows[gridRowIndex].Cells[3].Value = preco;
                        this.produtoDGV.Rows[gridRowIndex].Cells[4].Value = preco * quantidade;
                        //this.produtoDGV.Rows[gridRowIndex].Cells[4].Value = produto.PrecoVenda * quantidade;
                        this.produtoDGV.Rows[gridRowIndex].Cells[5].Value = precoAvista;
                        this.produtoDGV.Rows[gridRowIndex].Cells[6].Value = precoAvista * quantidade;
                    ...
                    {*/
                        ...
                        this.produtoDGV.Rows.Add(new object[] { produto.Id, produto.Nome, quantidade, preco, (preco * quantidade), precoAvista, (precoAvista * quantidade) });
                    //}
        ...
        private void produtoCodigoSistemaTB_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            Library.Produto produto = Library.ProdutoBD.FindById(tmpIdProduto);
